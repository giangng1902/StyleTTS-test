...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, tone, input_lengths, m):
        x = self.embedding(x)  # [B, T, 512]
        tone = self.embedding(tone)
        x = x+tone
...
class ProsodyPredictor(nn.Module):
    ...
    def __init__(self, style_dim, d_hid, nlayers, dropout=0.1):
        ...
        self.text_encoder = DurationEncoder(sty_dim=style_dim, #128
                                            d_model=d_hid, #512
                                            nlayers=nlayers, 
                                            dropout=dropout)
        ...
        self.shared = nn.LSTM(d_hid, d_hid // 2, 1, batch_first=True, bidirectional=True)
...
class DurationEncoder(nn.Module):
    ...
    def forward(self, x, style, text_lengths, m):
        ...
        zeros_tensor = torch.zeros((s.size(0), s.size(1), self.d_model - self.sty_dim), device='cuda:0')
        expanded_s = torch.cat((s, zeros_tensor), dim=-1)
        ...
        for block in self.lstms:
            if isinstance(block, AdaLayerNorm):
                ...
                # print(x.size(), expanded_s.permute(1, -1, 0).size())
                x = x + expanded_s.permute(1, -1, 0)
